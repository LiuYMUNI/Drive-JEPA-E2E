...
def configure_bev_ax_lower(ax: plt.Axes) -> plt.Axes:
    """
    Configure the plt ax object for birds-eye-view plots with the origin
    shifted towards the lower part of the view.
    :param ax: matplotlib ax object
    :return: configured ax object
    """
    ...
    # Shift origin downward: allocate more range in front (positive x) than behind (negative x)
    forward_frac = 0.75
    backward_frac = 1.0 - forward_frac
    ax.set_ylim(-margin_x * backward_frac, margin_x * forward_frac)

    # left is y positive, right is y negative
    ax.invert_xaxis()
