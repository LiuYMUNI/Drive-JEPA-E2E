...
class PDMScorer:
    ...
    def _aggregate_pdm_scores(self) -> npt.NDArray[np.float64]:
        ...
        max_raw_progress = np.max(combined_progress, axis=1)
        # three cases:
        ...
        normalized_progress[bigger_than_t_mask] = masked_progress[bigger_than_t_mask] / max_raw_progress[bigger_than_t_mask]
        self._weighted_metrics[WeightedMetricIndex.PROGRESS] = normalized_progress
    ...
    def _calculate_lane_keeping(self) -> None:
        ...
        # We use coarse estimation to speed up calculation.
        interval_length = 0.5 
        ...
        for proposal_idx in range(self._num_proposals):
            ...
            for time_idx in range(0, self.proposal_sampling.num_poses + 1, 5):
                ego_position = Point(
