...
def _disk_offsets(radius_px: int):
    """Integer (dy, dx) offsets inside a disk of radius_px."""
    if radius_px <= 0:
        return [(0, 0)]
    r = radius_px
    ...
    # coords are in [0..2r], shift to [-r..r]
    coords = coords - r
    return [tuple(c) for c in coords]  # (dy, dx)

def _binary_dilate_disk(mask: np.ndarray, radius_px: int) -> np.ndarray:
    """Binary dilation using OR over disk shifts. Pure NumPy."""
    if radius_px <= 0:
        return mask
    H, W = mask.shape
...
def _rasterize_polyline(points_xy: np.ndarray, x_min, y_min, res, H, W, sample_step_m):
    """
    Rasterize a polyline given by points_xy (T,2) into a thin boolean mask.
    Sampling along segments avoids gaps.
    """
    mask = np.zeros((H, W), dtype=bool)

    for k in range(len(points_xy) - 1):
        p0 = points_xy[k]
        p1 = points_xy[k + 1]
        seg = p1 - p0
        ...
        if L < 1e-9:
            ...
            n = int(np.ceil(L / sample_step_m)) + 1
            ts = np.linspace(0.0, 1.0, n, dtype=np.float32)
            xs = p0[0] + ts * seg[0]
            ys = p0[1] + ts * seg[1]

        # to pixel indices
        us = ((xs - x_min) / res).astype(np.int32)
        vs = ((ys - y_min) / res).astype(np.int32)
...
def miou_diversity_from_proposals(
    proposals: np.ndarray,
    line_width_m: float = 2.0,
    res_m_per_px: float = 0.2,
    margin_m: float = 5.0,
    return_pairwise: bool = False,
):
    """
    proposals: (N, T, 3) with (x, y, heading). Uses only x,y for IoU.
    line_width_m: full width in meters (buffer radius = width/2).
    res_m_per_px: BEV resolution, meters per pixel.
    margin_m: extra padding around min/max bounds.
    """
    assert proposals.ndim == 3 and proposals.shape[-1] == 3
    N, T, _ = proposals.shape
    xy = proposals[..., :2].astype(np.float32)

    # Bounds from data + margin + half-width so dilation doesn't clip
    half_w = 0.5 * line_width_m  # if you meant radius=2m, set half_w = line_width_m
    x_min = float(xy[..., 0].min() - margin_m - half_w)
    x_max = float(xy[..., 0].max() + margin_m + half_w)
    y_min = float(xy[..., 1].min() - margin_m - half_w)
    y_max = float(xy[..., 1].max() + margin_m + half_w)

    W = int(np.ceil((x_max - x_min) / res_m_per_px)) + 1
    H = int(np.ceil((y_max - y_min) / res_m_per_px)) + 1

    # Rasterize each proposal -> thin mask -> dilate to width
    radius_px = int(np.ceil(half_w / res_m_per_px))
    sample_step = 0.5 * res_m_per_px  # denser than grid to avoid holes

    masks = np.zeros((N, H, W), dtype=bool)
    for i in range(N):
        thin = _rasterize_polyline(xy[i], x_min, y_min, res_m_per_px, H, W, sample_step)
        thick = _binary_dilate_disk(thin, radius_px)
        masks[i] = thick

    # Pairwise IoU
    # (Compute sums once for speed)
    areas = masks.reshape(N, -1).sum(axis=1).astype(np.int64)

    ious = []
    if return_pairwise:
        pair_iou = np.ones((N, N), dtype=np.float32)

    for i in range(N):
        mi = masks[i]
        ai = areas[i]
        for j in range(i + 1, N):
            inter = np.logical_and(mi, masks[j]).sum(dtype=np.int64)
            union = ai + areas[j] - inter
            iou = float(inter / union) if union > 0 else 1.0
            ious.append(iou)
            if return_pairwise:
                pair_iou[i, j] = pair_iou[j, i] = iou

    ious = np.array(ious, dtype=np.float32)
    mean_iou = float(ious.mean()) if ious.size else 1.0
    p5_iou = float(np.percentile(ious, 5)) if ious.size else 1.0

    # Diversity is the complement of similarity
    diversity_mean = 1.0 - mean_iou
    diversity_p5 = 1.0 - p5_iou

    out = {
        "diversity_mean": diversity_mean,  # in [0,1]
        "diversity_p5": diversity_p5,      # anti-collapse, in [0,1]
        "mean_iou": mean_iou,
        "p5_iou": p5_iou,
        "grid_H": H,
        "grid_W": W,
        ...
        try:
            ...
            if agent.requires_scene:
                ...
                with torch.no_grad():
                    ...
                    proposals = predictions['proposals'].squeeze(0).cpu().numpy()
                    # pdm_score = predictions['pdm_score'].squeeze(0).cpu().numpy()
                    poses = predictions["trajectory"].squeeze(0).cpu().numpy()

                # extract trajectory
                trajectory = Trajectory(poses)
            
                diversity = miou_diversity_from_proposals(proposals)['diversity_mean']
