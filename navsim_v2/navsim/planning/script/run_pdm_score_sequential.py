...
def run_pdm_score(
    ...
    for a in args:
        ...
        # Sort tokens by their metric cache start_time
        token_start_times: Dict[str, float] = {}
        for token in tokens_to_evaluate:
            metric_cache_path = metric_cache_loader.metric_cache_paths[token]
            with lzma.open(metric_cache_path, "rb") as f:
                metric_cache: MetricCache = pickle.load(f)
            token_start_times[token] = metric_cache.timepoint.time_s

        tokens_to_evaluate = sorted(tokens_to_evaluate, key=token_start_times.get)
        
        past_start_time = 0 
        past_ego_simulated_states = None 
        for idx, (token) in enumerate(tokens_to_evaluate):
            ...
            try:
                ...
                with lzma.open(metric_cache_path, "rb") as f:
                    metric_cache: MetricCache = pickle.load(f)
                observation_interval = metric_cache.timepoint.time_s - past_start_time
                if observation_interval > 0.55:
                    past_ego_simulated_states = None
                ...
                if agent.requires_scene:
                    ...
                    trajectory = agent.compute_trajectory(agent_input, past_ego_simulated_states, metric_cache, observation_interval)
                ...
                past_start_time = metric_cache.timepoint.time_s
                past_ego_simulated_states = ego_simulated_states
            except Exception:
